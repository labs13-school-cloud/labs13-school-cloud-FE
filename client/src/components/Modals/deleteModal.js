...
class TrainingSeriesModal extends React.Component {
	...
	handleDelete = () => {
		switch (this.props.deleteType) {
			...
			case 'removeMemberFromTS':
				this.props.deleteTeamMemberFromTrainingSeries(this.props.id, this.props.training_series_id);
				break;
			case 'trainingSeries':
				this.props.deleteTrainingSeries(this.props.id, this.props.user_id);
				break;
			case 'user':
	...
	handleDisplayType = () => {
		switch (this.props.displayType) {
			case 'button':
				return (
					<Button variant="primary" style={{ marginLeft: 10 }} onClick={this.handleOpen}>
						Delete
					</Button>
				);
			case 'text':
			...
			default:
				...
				return (
					<i onClick={this.handleOpen} className={`material-icons ${classes.icons}`}>
						delete
					</i>
				);
		}
