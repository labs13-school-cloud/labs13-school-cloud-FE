...
class AssignMemberPage extends React.Component {
	...
	renderTrainingSeriesInDropDown = () => {
		...
		return filteredSeries.map((series) => {
			return (
				<MenuItem name="trainingSeriesID" label={`${series.title}`} value={series.trainingSeriesID}>
					{series.title}
				</MenuItem>
			);
		});
				...
				<Paper className={classes.paper}>
					...
					>
						...
						<ButtonContainer>
							<Button
								disabled={
									this.state.isRouting === true || this.state.trainingSeriesID === undefined ? (
										'true'
									) : null
								}
...
export default connect(mapStateToProps, { addTeamMemberToTrainingSeries })(withStyles(styles)(AssignMemberPage));
