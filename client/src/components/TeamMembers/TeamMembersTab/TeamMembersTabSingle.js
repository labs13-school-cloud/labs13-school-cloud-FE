...
function TeamMembersTabSingle(props) {
    ...
    useEffect(() => {
        axios
            .get(`${process.env.REACT_APP_API}/api/team-members/${props.teamMember.mentor}`)
            .then(res => {
                setMentor(res.data.teamMember)
            })
            .catch(err => { //I know, this is a hilarious way to do this, but I think it looks a bit cleaner over all hahaha. Change if you want xD
                setMentor({
    ...
    useEffect(() => {
        axios
            .get(`${process.env.REACT_APP_API}/api/team-members/${props.teamMember.manager}`)
            .then(res => {
...
export default connect(null, {})(TeamMembersTabSingle);
