...
class AssignMemberPage extends React.Component {
	...
	componentDidUpdate(prevProps, prevState) {
		if (prevProps.isEditing && this.state.start_date !== this.prevState.start_date) {
			console.log('CDU');
	...
	renderTrainingSeriesInDropDown = () => {
		...
		let assignments = this.props.location.state.assignments.map((assignment) => assignment.trainingSeries_ID);
		...
		return filteredSeries.map((series) => {
			return (
				<MenuItem name="trainingSeriesID" label={`${series.title}`} value={series.training_series_id}>
					{series.title}
				</MenuItem>
			);
		});
				...
				<Paper className={classes.paper}>
					...
					>
						<FormControl className={''}>
							<InputLabel htmlFor="trainingSeriesID">Training Series</InputLabel>
							<Select
								value={this.state.training_series_id}
								onChange={this.handleChange}
								name="trainingSeriesID"
							>
								{this.props.location.state.assignments && this.renderTrainingSeriesInDropDown()}
							</Select>
						</FormControl>
						<ButtonContainer>
							<Button
								disabled={
									this.state.isRouting === true || this.state.training_series_id === undefined ? (
										'true'
									) : null
								}
...
export default connect(mapStateToProps, {})(withStyles(styles)(AssignMemberPage));
