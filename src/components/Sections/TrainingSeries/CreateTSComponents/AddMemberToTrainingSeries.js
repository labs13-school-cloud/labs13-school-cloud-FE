...
import { getTrainingSeriesMessages, getTeamMembers, getTrainingSeries, addNotification } from 'store/actions';
...
function AddMemberToTrainingSeries(props) {
	...
	const { getTrainingSeriesMessages, getTeamMembers, getTrainingSeries, match, userId } = props;
	...
	useEffect(
		...
		[ getTrainingSeriesMessages, getTeamMembers, getTrainingSeries, userId, id ]
	);
		...
		<Wrapper>
			<h1>
				{props.trainingSeries.length &&
					props.trainingSeries.filter((series) => parseInt(series.id) === parseInt(props.match.params.id))[0]
						.title}
			</h1>
			<p>Employee's will be sent {props.messages.length} message(s) throughout this training series.</p>
			<div>
				{props.teamMembers.map((member) => {
					return <SingleMemberCheck addMember={addMember} key={member.id} teamMember={member} />;
				})}
			</div>
			<form noValidate>
				<TextField
				...
				onClick={(e) => {
					...
					activeMembers.map((memberID) => {
						props.messages.map((msg) => {
							...
							const memberServices = props.teamMembers.filter((mem) => mem.id === memberID);
							const newNotification = {
								team_member_id: memberID,
								service_id: memberServices[0].phone_number ? 1 : memberServices[0].email ? 2 : 3,
								message_id: msg.id,
								num_attempts: 0,
								is_sent: false,
								send_date: moment(startDate).add(msg.days_from_start, 'days').toISOString()
							};
